...
```{python}
pops_drop = ["ASW", "ESN", "CHS", "CEU", "GIH"]
```

```{python}
dist_mat_pd2 = dist_mat_pd2.drop(pops_drop, axis = 1)
```

```{python}
rows_drop = ~dist_mat_pd2["pop"].isin(pops_drop)
rows_drop.values
```

```{python}
dist_mat_pd2 = dist_mat_pd2[rows_drop.values]
```
...
```{python, eval = F}
GT(dist_mat_pd2).tab_style(style=style.text(size = "0"),locations=loc.body(columns=num_cols))
```
...
# make my table
(
    GT(dist_mat_pd2).tab_style(
        style=style.text(size = "1"),
